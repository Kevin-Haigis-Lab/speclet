...
read_replicate_map <- function(path) {
  read_csv(path) %>%
    janitor::clean_names() %>%
...
assert_only_one_possible_final_count_column <- function(counts_df, possible_cols) {
  if (length(possible_cols) == 1) {
    ...
    print(head(counts_df), n = 5)
    cols <- paste(possible_cols, collapse = ", ")
    print(glue("possible columns: {cols}"))
...
save_processed_read_counts <- function(df, out_file) {
  write_csv(df, out_file, append = file.exists(out_file))
  return(NULL)
}
...
process_score_read_count_replicate <- function(replicate_id,
                                               ...
                                               counts_dir,
                                               out_file) {
  info(logger, glue("Processing raw read counts for '{replicate_id}'"))
  read_ct_file <- paste0(replicate_id, ".read_count.tsv.gz")
  read_ct_path <- file.path(counts_dir, read_ct_file)
  info(logger, glue("read count path: '{read_ct_path}'"))
  read_ct_df <- read_count_file(read_ct_path) %>%
    add_column(depmap_id = depmap_id) %>%
    dplyr::relocate(depmap_id, hugo_symbol, sgrna) %>%
    arrange(hugo_symbol, sgrna)

  info(logger, glue("number of rows in read count data frame: {nrow(read_ct_path)}"))

  if (!p_dna_batch %in% colnames(read_ct_df)) {
    stop(glue("pDNA batch column not found ({p_dna_batch})"))
  }

  read_ct_df <- read_ct_df %>%
    rename(p_dna_counts = {{ p_dna_batch }})

  final_counts_col <- colnames(read_ct_df)[str_detect(colnames(read_ct_df), "CRISPR")]
  ...
  info(logger, glue("final count column: '{final_counts_col}'"))
...
x <- read_replicate_map(replicate_map_path) %>%
